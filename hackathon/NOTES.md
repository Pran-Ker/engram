...
Working notes from the day. Kept for reference; the product docs live in [`../engram/README.md`](../engram/README.md).
...
**Deadline:** project submission 4:30 PM PT at https://bit.ly/long-horizon-hack (one project per team, up to 4 people). Judging 4:45, finalists 6:00, awards 7:00. Everyone must be registered on Luma + AWS Builder (http://events.builder.aws.com/d/xdz2bp). Submission copy: [`submission.md`](submission.md).
...
RawTree fits the "agents that don't drown in their own history" theme: log every agent step, tool call, and test result as an event, then have the agent query its own history instead of carrying it in context.
...
Keys live in `~/.local/secrets` (never in this repo). Copy `.env.example` if you need a local `.env`.

```bash
npm install
source ~/.local/secrets
npm run check      # smoke-tests all four sponsors, one line per sponsor
```
...
- The 8B MoE `ollama run lfm2.5` (tool calling, 128K ctx) needs Ollama ≥ 0.17.
- Fastest on Apple Silicon: `pip install mlx-lm && mlx_lm.server --model LiquidAI/LFM2.5-2.6B-MLX-8bit --port 8080`.
- API fallback: OpenRouter `liquid/lfm-2.5-*` models; look for the `:free` ones.
...
- **Credits:** no free tier. BFL gave $100 per account for the hackathon, requested through the sponsor's Discord thread.
...
- Access via the hackathon invite https://tbrd.co/tokensand (Google login). Console: https://rawtree.com/tokensand
...
- MCP: `claude mcp add --transport http rawtree https://mcp.rawtree.com/mcp --header "Authorization: Bearer $RAWTREE_API_KEY"`. Agent skills: `npx skills add rawtreedb/agent-skills`. Docs: https://rawtree.com/docs, OpenAPI https://api.rawtree.com/v1/openapi.json

## Discord

Server: Tokens& Hackathon. Channels under "Sept. 25 - Long Horizon Agents Hack": `#nimble`, `#liquidai`, `#black-forest-lab` (+ credits thread), `#tinybird`, and `#ask-organizers`.
